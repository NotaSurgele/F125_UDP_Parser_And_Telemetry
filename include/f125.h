...
{
    uint16 m_speed;                      // Speed of car in km/h
    float    m_throttle;                   // Amount of throttle applied (0.0 to 1.0)
    float    m_steer;                      // Steering (-1.0 to 1.0)
    ...
    uint16 m_revLightsBitValue;           // Rev lights (bit 0 = leftmost LED)
    uint16 m_brakesTemperature[4];       // Brakes temperature (°C)
    uint8  m_tyresSurfaceTemperature[4]; // Tyres surface temperature (°C)
    uint8  m_tyresInnerTemperature[4];   // Tyres inner temperature (°C)
    uint16 m_engineTemperature;           // Engine temperature (°C)
...
{
    PacketHeader m_header;                    // Header
    CarTelemetryData m_carTelemetryData[22]; // Telemetry for all cars
    uint8 m_mfdPanelIndex;                  // Index of MFD panel open (255 = closed)
    uint8 m_mfdPanelIndexSecondaryPlayer;  // Secondary player MFD
    int8 m_suggestedGear;                  // Suggested gear
};
#pragma pack(pop)
